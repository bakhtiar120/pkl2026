...
<div class="container-fluid">
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb pl-0">
            <li class="breadcrumb-item"><a href="/admin/nilai-peserta">Nilai Peserta</a></li>
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h5 mb-0 text-gray-800">Lihat Nilai</h1>
    </div>
            ...
            <div class="card shadow-new mb-4">
                <div class="card-body">
                    <div class="table-responsive mt-4">
                                <table class="table table-hover table-bordered example" id="dataTable" width="100%" cellspacing="0">
                                    <thead>
                                        <tr>
                                    ...
                                    <tbody>
                                      ...
                                      <tr>
                                        ...
                                        <td>@if($member1->skor_integritas) {{$member1->skor_integritas}}@endif</td>
                                        <td>@if($member1->skor_pengembagan_diri) {{$member1->skor_pengembagan_diri}}@endif</td>
                                        <td>@if($member1->skor_kreatifitas) {{$member1->skor_kreatifitas}}@endif</td>
                                        <td>@if($member1->skor_komunikasi) {{$member1->skor_komunikasi}}@endif</td>
                                        <td>@if($member1->skor_analisis) {{$member1->skor_analisis}}@endif</td>
                                        <td>@if($member1->skor_kerja_sama) {{$member1->skor_kerja_sama}}@endif</td>
                                        <td>@if($member1->skor_pemahaman) {{$member1->skor_pemahaman}}@endif</td>
                                        <td>@if($member1->skor_presentasi) {{$member1->skor_presentasi}}@endif</td>
                                        <td>
                                            <a href="/admin/detail-profile/{{ $member1->id_member }}"><button type="button" class="btn btn-sm btn-outline-new">Detail</button></a>
                                            <button data-member='{{json_encode($member1)}}' type="button" class="btn btn-sm btn-outline-new update-scor">Nilai</button>
                                        </td>
                                      </tr>
                                      @endforeach
                                    </tbody>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('.save').on( 'click',function () {
            $.post( "/admin/api/update-skor",$("#formUpdateDataScore").serializeObject()).done(function(data){
